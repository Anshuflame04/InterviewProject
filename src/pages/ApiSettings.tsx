...
import { LLM_MODELS, getLlmSettingsByProvider, loadLlmSettingsFromFirebase, saveLlmSettingsToFirebase, type LlmProvider } from "../services/llmSettings";
...
export default function ApiSettings() {
    ...
    useEffect(() => {
        void loadLlmSettingsFromFirebase().then((settings) => {
            setSavedSettings(settings);
            const current = settings.gemini ?? settings.groq;
            if (current) {
                setModel(current.model);
                setApiKey(current.apiKey);
            }
        }).catch((error: unknown) => setSaveError(error instanceof Error ? error.message : "Could not load saved settings."));
    }, []);
    ...
    const save = async () => {
        ...
        setSaveError("");
        try {
            await saveLlmSettingsToFirebase(settings);
            window.dispatchEvent(new Event("llm-settings-changed"));
            setSavedSettings((current) => ({ ...current, [provider]: settings }));
            setSavedMessage(true);
        } catch (error) {
            setSaveError(error instanceof Error ? error.message : "Could not save API configuration.");
        }
    };
    ...
    return <div className="max-w-2xl space-y-6">
        ...
        <section className="rounded-2xl border border-white/10 bg-slate-900 p-5 sm:p-6">
            ...
            <p className="mt-3 text-xs leading-5 text-cyan-200/80">Saving also synchronizes this configuration to your signed-in Firebase account, protected by owner-only Firestore rules.</p>
            <div className="mt-6 grid gap-5">
                ...
                <button type="button" onClick={() => void save()} disabled={!apiKey.trim()} className="inline-flex w-fit items-center gap-2 rounded-xl bg-cyan-400 px-5 py-3 text-sm font-bold text-slate-950 disabled:opacity-40"><Save size={16} /> Save configuration</button>
        ...
        <section className="rounded-2xl border border-white/10 bg-slate-900 p-5 sm:p-6">
            <h2 className="font-bold text-white">How to use InterviewYou</h2>
